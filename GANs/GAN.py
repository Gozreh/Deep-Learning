...
# show images for select epochs
showepoch=[1,10,20,50,100]

class Generator(nn.Module):

    """ Image generator. Takes a noise vector as input and syntheses a single channel image accordingly """

    def __init__(self, input_dims, output_dims):

        """Init function. Declare the network structure.
...
class Discriminator(nn.Module):
    ...
    """ Image discriminator. Takes a image as input and predict if it is real from the dataset or fake synthesised by the generator """

    def __init__(self, input_dims, output_dims=1):

        """ Init function. Declare the discriminator network structure.
...
def show_train_hist(hist, show=False, save=False, path='Train_hist.png'):
    ...
    """Loss tracker. Plot the losses of generator and discriminator independently to see the trend
...
def show_result(G_net,z_,epoch, show=False, save=False, path='results.png'):
    ...
    grid_img=make_grid(outputs)
    print(grid_img.shape)

    if save:
        #path=f'results{epoch}.png'
        save_image(grid_img,path)
    if show:
        image=grid_img.permute(1,2,0)
        image=image.numpy()
        plt.imshow(image)
        plt.show()
    else:
    ...
    """ m= nn.Sequential(nn.Linear(784, 784),nn.Unflatten(-1, torch.Size([1, 28, 28])))
    genim = G_net(create_noise(100, G_input_dim).to(device)).cpu().detach()
    unfimg=m(genim)
    print(unfimg.shape)
    #generated_img = G_net(z_).cpu().detach()
    grid_img = make_grid(unfimg)
    #print(grid_img.shape)
    save_image(grid_img,f"gen_img{epoch}.png")
    #img=generated_img.numpy()
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(device)
    data_dir = './MNIST_data/'  # path to MNIST dataset
    save_dir = './MNIST_GAN_results/'   # save path for loss graphs etc.
    image_save_dir = './MNIST_GAN_results/results'  # save path for images
    ...
    for epoch in range(epochs):
        ...
        if epoch +1 in showepoch:
            path = image_save_dir + '/MNIST_GAN_' + str(epoch + 1) + '.png'
            show_result(G_net, create_noise(25, 100).to(device), (epoch + 1), save=True, show=False, path=path)
